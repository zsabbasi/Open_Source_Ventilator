...
static uint8_t rowIndex, colIndex;
...
//  }
    ...
    //------- update a character -------
    //LOGV("rowIndex=%d, colIndex=%d", rowIndex, colIndex);
    sendFast(*fbPtr++, 1);
    colIndex++;
    ...
    if (colIndex >= numCol) {
        ...
        //LOGV("Set addr = 0x%x", _row_offsets[rowIndex]);
        fbPtr = fbSourceRows[rowIndex];
        //LOGV("Set fbPtr = 0x%x\n", fbPtr);
        delayMicroseconds(1);
        sendFast(LCD_SETDDRAMADDR | (_row_offsets[rowIndex]), 0); // zero = command
    }
}
